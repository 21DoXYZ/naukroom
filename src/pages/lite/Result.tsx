...
export default function LiteResult() {
  ...
  function copyBio() {
    const bio = audit?.bioVariants[activeBioIdx]
    if (!bio) return
  ...
  const weakScores = audit.scores.filter(s => s.label !== 'Добре')
  const strongScores = audit.scores.filter(s => s.label === 'Добре')
      ...
      <div className="max-w-[640px] mx-auto px-5 py-10 flex flex-col gap-5">
        ...
        <motion.div {...spring(0)}>
          ...
          <h1 className="text-[1.75rem] fw-400 tracking-[-0.7px] leading-[1.15] mb-1">
            Ось що ми побачили у вашому профілі
          </h1>
          {input?.profession && (
            <p className="type-body text-[rgba(0,0,0,0.45)] mb-4">{input.profession}</p>
          )}

          {/* Score pill + summary */}
          <div className="flex items-start gap-4 p-4 rounded-[8px] bg-black/[0.02] border border-black/8">
            <div className="flex flex-col items-center shrink-0">
              <span
                className="text-[2rem] fw-540 tracking-[-1px] leading-none"
                style={{ color: scoreColor }}
              >
                {audit.overallScore}
              </span>
              <span className="text-[0.75rem] text-[rgba(0,0,0,0.35)] mt-0.5">/ 100</span>
            </div>
            <p className="type-body text-[rgba(0,0,0,0.7)] leading-[1.55]">{audit.summary}</p>
          </div>
        </motion.div>
        ...
        <motion.div {...spring(1)} className="rounded-[8px] border border-black/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-black/8">
            <p className="type-mono-label text-[rgba(0,0,0,0.4)]">Деталі аудиту</p>
          </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex items-center gap-2 shrink-0">
                      <span
                        className="text-[0.75rem] fw-540 tabular-nums"
                  ...
                  <span
                    className="text-[0.7rem] fw-500 px-2 py-0.5 rounded-full shrink-0"
                ...
                {expandedScore === i && (
                  <div className="mt-3 flex flex-col gap-2.5 text-left">
                    <div className="flex items-start gap-2">
                      <AlertCircle className="h-3.5 w-3.5 mt-0.5 shrink-0" style={{ color: labelColor(s.label) }} />
                      <p className="text-[0.8375rem] text-[rgba(0,0,0,0.7)] leading-[1.5]">{s.issue}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <CheckCircle2 className="h-3.5 w-3.5 mt-0.5 shrink-0 text-[oklch(0.56_0.17_155)]" />
                      <p className="text-[0.8375rem] text-[rgba(0,0,0,0.75)] leading-[1.5] fw-430">{s.fix}</p>
                    </div>
                  </div>
                )}
              </button>
          ...
          <div className="px-5 py-3 border-t border-black/6">
            <p className="text-[0.8125rem] text-[rgba(0,0,0,0.4)]">
              Натисніть на категорію, щоб побачити деталі та рекомендацію
            </p>
          </div>
        </motion.div>

        {/* Problems (weak scores) */}
        {weakScores.length > 0 && (
          <motion.div
            {...spring(2)}
            className="rounded-[8px] border border-black/10 overflow-hidden"
            style={{ borderTopColor: 'oklch(0.60 0.20 22)', borderTopWidth: 3 }}
          >
            <div className="px-5 py-4 border-b border-black/8">
              <p className="type-mono-label text-[rgba(0,0,0,0.4)]">Що заважає профілю продавати</p>
            </div>
            <div className="flex flex-col divide-y divide-black/6">
              {weakScores.map((s, i) => (
                <div key={i} className="flex items-start gap-3 px-5 py-4">
                  ...
                  <div>
                    <p className="text-[0.8125rem] fw-500 text-[rgba(0,0,0,0.5)] mb-0.5">{s.category}</p>
                    <p className="type-body text-[rgba(0,0,0,0.8)]">{s.issue}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="px-5 py-3 border-t border-black/6">
              <p className="text-[0.8125rem] text-[rgba(0,0,0,0.4)]">Це не критика - це точки зростання</p>
            </div>
          </motion.div>
        )}
        ...
        {strongScores.length > 0 && (
          <motion.div
            {...spring(3)}
            className="rounded-[8px] border border-black/10 overflow-hidden"
            style={{ borderTopColor: 'oklch(0.56 0.17 155)', borderTopWidth: 3 }}
          >
            <div className="px-5 py-4 border-b border-black/8">
              <p className="type-mono-label text-[rgba(0,0,0,0.4)]">Що вже працює</p>
            </div>
            {strongScores.map((s, i) => (
              <div key={i} className="flex items-start gap-3 px-5 py-4">
                ...
                <div>
                  <p className="text-[0.8125rem] fw-500 text-[rgba(0,0,0,0.5)] mb-0.5">{s.category}</p>
                  <p className="type-body text-[rgba(0,0,0,0.8)]">{s.fix}</p>
                </div>
              </div>
            ))}
          </motion.div>
        ...
        {/* Rewritten Bio - switchable variants */}
          ...
          >
            ...
            {audit.bioVariants.length > 1 && (
              <div className="flex px-5 pt-4 gap-2">
                {audit.bioVariants.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => { setActiveBioIdx(i); setBioCopied(false) }}
                    className={[
                      'h-7 px-3 rounded-full text-[0.75rem] fw-450 transition-colors cursor-pointer',
                      activeBioIdx === i
            ...
            <div className="px-5 py-4">
              <p className="type-body fw-450 text-[rgba(0,0,0,0.85)] leading-[1.65]">
                {audit.bioVariants[activeBioIdx]}
              </p>
            </div>
            <div className="px-5 py-3 border-t border-black/6">
              <p className="text-[0.8125rem] text-[rgba(0,0,0,0.4)]">
                Адаптуйте під себе - замініть деталі під вашу аудиторію
              </p>
            </div>
          </motion.div>
        )}
        ...
        {audit.quickWins.length > 0 && (
          ...
          >
            ...
            <div className="px-5 py-3 border-t border-black/6">
              <p className="text-[0.8125rem] text-[rgba(0,0,0,0.4)]">Кожна дія займає до 30 хвилин</p>
            </div>
          </motion.div>
        )}

        {/* Pinned post / Reels idea */}
        {audit.pinnedPostIdeas.length > 0 && (
          ...
          >
            <div className="px-5 py-4 border-b border-black/8">
              <p className="type-mono-label text-[rgba(0,0,0,0.4)]">Ідеї для закріплених постів</p>
            </div>
            <div className="flex flex-col divide-y divide-black/6">
        ...
        <motion.div {...spring(8)} className="rounded-[8px] border border-black/10 p-5">
          ...
          <p className="text-[0.8125rem] text-[rgba(0,0,0,0.4)]">
            Lite - вхідна точка. Повна версія - весь маркетинг-пак під вашу нішу.
          </p>
        </motion.div>

        {/* Next steps - 4-way intent routing */}
        <motion.div {...spring(9)}>
          <div className="mb-4">
        ...
        >
          ...
          Повний аналіз включає онбординг і перевірку Instagram-профілю.
        </motion.p>
      </div>
    </div>
  )
